...
%	Assertion \eqref{thm:yaglom} of Theorem \ref{thm:kesten} 
...
%conceptual proof of Theorem \ref{thm:kesten} 
probabilistic proof of Theorem \ref{thm:kesten} 
%using the size-biased Galton-Watson tree.
...
\end{equation*}
%The celebrated \defn{size-biased Galton-Watson tree} is 
...
%Now we first present a $k(k-1)$-type size-biased Galton-Watson tree equipped 
...
%Suppose that the offspring distribution $\mu$ satisfies \eqref{eq:mean} and \eqref{eq:variance}. 
...
%two-spine model is to characterize the $k(k-1)$-type size-biased branching process.
...
%below, and give a new probabilistic proof of Theorem 1(2).
...
%Galton-Watson tree, the population of the particles off the spine 
...
%Galton-Watson tree. This decomposition leads us to another
...
%	Galton-Watson tree $\dot T$ and its distinguishable spine $\dot V$ 
...
\end{align*}
\par	
%	The \defn{$k(k-1)$-type size-biased Galton-Watson measure 
...
%		Galton-Watson process stopped at generation $n$, we have that
...
%    $k(k-1)$-type size-biased Galton-Watson tree with height $n$, we have that
...
%	\eqref{thm:kolmogorov} of Theorem \ref{thm:kesten},
