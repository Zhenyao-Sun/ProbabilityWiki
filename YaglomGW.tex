%%%----Versions-----------------------------
%---YaglomGW18.tex 2018/5/11  by Zhenyao-------
%---YaglomGW18.tex 2018/5/10  by Renming-------
...
%end new added
	As a consequence,  
	%in order to show Yaglom's theorem, 
...
\]
	%where $\dot Y$ is the $Y$-transform 
	...
	%the argument of \cite{geiger1999elementary} 
...
\end{equation}
	...
	%Therefore, if $\dot Z_n/n$ converges weakly to, say, $\dot Y$, then $\dot Y$ should satisfy \eqref{eq: x2 type size-biased equation for exponential distribution}, which, by Lemma \ref{lem: our equation}, suggests that \eqref{eq: convergence after size-biased} is true.
...
\end{equation}
	%and therefore, if $Y$ is the weak limit of $Z_n/n$ conditioned on $\{Z_n > 0\}$, then $Y$ should satisfy \eqref{eq: Lyons' distributional equation}, which suggests that $Y$ is exponentially distributed.
	...
	%	If $Y^{(1)}$ and $Y^{(2)}$ are independent copies of an exponential random variable $Y$, and $U$ is  an independent uniform random variable on $[0,1]$, then
...
%ZY: Yes, it is. In his article, Geiger did't point out this. However, from his proof of exponential convergence, using a similar L2-wasserstein type argument, one can show that (1.12) implies that Y is exponentially distributed. 
...
%\[
    ...
    %\quad \mathbf t \in \mathbb T_{\leq n}.
%\]
...
%deleted
	%Then $\dot T=\bigcup_{k=0}^n\dot A_k.$ 
...
\]
	Then, 
%deleted	
	%the particles in $\ddot T$ can be separated according to:
%\[\begin{split}
	%\ddot T
