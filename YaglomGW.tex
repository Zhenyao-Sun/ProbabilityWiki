%%%----Versions-----------------------------
%---YaglomGW17.tex 2018/5/8  by Yanxia--------
%---YaglomGW16.tex 2018/5/7  by Zhenyao--------
%---YaglomGW15.tex 2018/3/9 submitted to PCE and Arxiv.org by Zhenyao
%---YaglomGW15.tex 2018/3/7 by Zhenyao----
...
\cite{geiger1999elementary}, \cite{geiger2000new} and \cite{lyons1995conceptual}.
	...
	Let $\{X;P\}$ be a	random variable
	and $g(X)$ be a Borel function of $X$ with $P(g(X) \geq 0) = 1$ and $E[g(X)]\in (0,\infty)$.
	We say a	random variable
	$\dot X$ is a $g(X)$-size-biased transform (or simply $g(X)$-transform) of $X$ if
	\[
		E[f(\dot X)] = \frac{ E[g(X)f(X)]}{E[g(X)]}
	\]
	for each positive Borel function $f$.
%new
A $X$-size-biased transform of of $X$ is sometimes simply called a size-biased transform of of $X$.
%end new
...
%	independent birth to a random number of unmarked children according to $L$.
...
\item
	There is an initial particle which is marked.
\item
	Before or after generation $K_n$, any marked particle 
	%gives independent birth to a random number of children according to $\dot L$.
	gives birth independently to a random number of children according to $\dot L$.
	Pick one of those children randomly as the new marked particle while leaving the other children as unmarked particles.
\item
	The marked particle at generation $K_n$, however, 
	%gives independent birth to a random number of children according to $\ddot L$.
	gives birth, independent of other particles, to a random number of children according to $\ddot L$.
...
\subsection{Methods.}
\label{sec: Methods}
	Suppose that $X$ is a non-negative random variable, then its distribution conditioned on $\{ X > 0\}$ can be characterized by its conditional expectation $E[X|X>0]$ and its size-biased transform $\dot X$.
...
\end{split}
\end{equation}
	As a consequence, in order to show Yaglom's theorem, 
...
\end{equation}
	where $\dot Y$ is the $Y$-transform of the exponential random variable $Y$.
	Note that, since $E[Z_n] = 1$, \eqref{eq: convergence of conditional expectation} is equivalent to Theorem \ref{thm: Kolmogrov and Yaglom theorem}(\ref{thm:kolmogorov}).
	In Section \ref{sec: proofs}, for completeness, we will simplify the argument 
%	made by \cite{geiger1999elementary} and give a proof of 
of \cite{geiger1999elementary} and give a proof of 
Theorem \ref{thm: Kolmogrov and Yaglom theorem}(\ref{thm:kolmogorov}).

	As mentioned above, our method of proving
...
\end{lem}	
	...
	From the construction of the $k(k-1)$-type size-biased $\mu$-Galton-Watson tree 
%	$(\ddot Z^{(n)}_m)$, we see that the population 
	$(\ddot Z^{(n)}_m)_{0\le m\le n}$, we see that the population 
	$\ddot Z^{(n)}_n$ in
...
%	Therefore, if the weak limit of $\dot Z_n/n$ exists, say $\dot Y$, 
Therefore, if $\dot Z_n/n$ converges weakly to, say, $\dot Y$, 
	then $\dot Y$ should satisfy \eqref{eq: x2 type size-biased equation for exponential distribution}, which, by Lemma \ref{lem: our equation}, suggests that \eqref{eq: convergence after size-biased} is true.
	
	It is interesting to compare
%	this view of exponential convergence with the methods 
    this method of proving exponential convergence with the methods 
...
\end{equation}
	...
	In \cite{geiger2000new}, Geiger characterizes the exponential distribution by another distributional equation:
		...
		and $U$ is  an independent uniform random variable on $[0,1]$, then
\begin{equation}
\label{eq: Geiger's equation}
	Y	\overset{d} = U (Y^{(1)} + Y^{(2)}).
\end{equation}
({\bf Is the a characterization? Please check \cite{geiger2000new}}.)
...
\end{equation}
	...
	From this comparison, we see that all the methods mentioned above 
%	shares one similarity: They all establish the exponential convergence 
  share one similarity: They all establish the exponential convergence 
	via some particular distributional equation.
...
\end{equation}
%	From this point of view, after considering the Laplace transform of 
...
\begin{proof}[Proof of Theorem \ref{thm: change of measure}]
    Note that 
    % $\{(X_m( \mathbf t ))_{0\le m\le n}; {\mathbf G}_n\}  \overset{d}{=} (Z_m)_{0\le m\le n}$ and $\{(X_m( \mathbf t ))_{0\le m\le n};\ddot{\mathbf G}_n\}  \overset{d}{=} (\ddot Z_m)_{0\le m\le n}$.
    \[
...
\begin{equation*}
    \frac{\ddot{\mathbf G}_n(\mathbf t )}{\mathbf G_n (\mathbf t ) }
...
\]
	Then, the particles in $\ddot T$ can be separated according to:
\begin{equation*}\begin{split}
	\ddot T
	=	\Big ( \bigcup_{k=0}^n \ddot A^l_k\Big)  \cup \Big( \bigcup_{k=K_n+1}^n \ddot A^s_k \Big).
\end{split}\end{equation*}
%Delete the display above?
...
\begin{equation*}  \begin{split}
	&E [ e^{- \lambda \ddot Z_n^{(n)}} ]
	= \frac{1}{n}\sum_{m=0}^{n-1} \Big( \prod_{k=0,k\neq m}^{n} E[e^{-\lambda Z^{(\dot L - 1)}_{n-k-1}}] \Big) \cdot E [e^{-\lambda Z^{(\ddot L - 2)}_{n-m-1}}] \cdot 
	\Big(\prod_{k= m+1}^n E [e^{-\lambda Z^{(\dot L - 1)}_{n-k-1}}]\Big)
...
\\&\quad = E [e^{-\lambda \dot Z_n}]  \frac{1}{n}\sum_{m=0}^{n-1}   \frac{ E [e^{-\lambda Z^{(\ddot L - 2)}_{n-m-1} }] }{ E[e^{-\lambda Z^{(\dot L - 1)}_{n-m-1}  }] } \cdot E[e^{- \lambda \dot Z_{n-m-1}}]\\
%	= E [e^{-\lambda \dot Z_n}]  \frac{1}{n}\sum_{m=0}^{n-1}   
\\&\quad = E [e^{-\lambda \dot Z_n}]  \frac{1}{n}\sum_{m=0}^{n-1} 
	\frac{ E [e^{-\lambda Z^{(\ddot L - 2)}_{m}}] }{ E[e^{-\lambda Z^{(\dot L - 1)}_{m}}] } \cdot E[e^{- \lambda \dot Z_{m}}]
	\\&\quad = E [e^{-\lambda \dot Z_n}] E[g(\lambda, \floor{Un})e^{-\lambda \dot Z_{\floor{Un}}}],
\end{split}
\end{equation*}
	where
\[
...
\end{split}\]
	Here, $q_n = P[Z_n = 0] = f^{(n)}(0)$, $p_n = 1- q_n$ and $f$ is the probability generating function of the offspring distribution $\mu$.
...
\]
	Therefore,
	%Give an equation number to the first display of this proof, then change `Therefore' to `Now combining eqn # with above, we get'
\[\begin{split}
	&\frac{1}{n P(Z_n > 0)}
...
%In order to make comparison between 
In order to compare 
distributions using their size-biased add-on structures, we need the following lemma:
...
%	As have been discussed in Section \ref{sec: Methods}, 
