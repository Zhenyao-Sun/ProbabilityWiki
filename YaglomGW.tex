%%%----Versions-----------------------------
%---YaglomGW13.tex 2018/3/4 by Zhenyao
%---YaglomGW12.tex 2018/3/4 by Zhenyao
...
\end{equation}
	...
	%It is natural to consider probabilistic proofs of analogous results for more general critical branching processes. 
...
\label{eq: x2 type size-biased equation for exponential distribution}
	...
	= \dot Y + U \cdot \dot Y'.
\end{equation}
%v10%	Here, $\dot Y$ and $\dot Y'$ are independent $Y$-transforms of $Y$; $\ddot Y$ is the $Y^2$-transform of $Y$; and $U$ is an independent uniform random variable on $[0,1]$.
%v11%	Here $\dot Y$ is the $Y$-transform of  $Y$, $\ddot Y$ is the $Y^2$-transform of $Y$, $U$ is a uniform random variable on $[0,1]$, and $\dot Y$, $\ddot Y$ and $U$ are independent.
...
\end{equation}
	...
	However, it seems that this tightness argument is not suitable for \eqref{eq: Geiger's insight}, due to a property that the conditional convergence for some subsequence $Z_{n_k}/n_k$ implies the convergence of $U \cdot \dot Z_{n_k}/n_k$, %but dose not implies the convergence of $Z^{(i)}_{[Un_k]}/Un_k, i=1, 2$. 
...
%\end{equation}
	%It is elementary to verify that, if $\mathbf e$ is an exponential random variable with mean $a/2$, and if $\dot {\mathbf e}$ is the $\mathbf e$-transform of $\mathbf e$, then the Laplace exponent of $\dot {\mathbf e}$ can also be expressed as
...
%\[\begin{split}
	...
	%\\&\quad \leq \frac{\lambda }{n} + \sigma^2 \int_0^\lambda ds \int_0^1 \big| g(\frac{s}{n}, \floor{un}) E[e^{-su \frac { Z_{\floor{un}} } {un} }] - E[e^{- su \dot Y}]\big| du 
%\end{split}\]
\[
