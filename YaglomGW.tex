...
\end{split}
\end{equation}
	As a consequence, in order to show the Yaglom's theorem, we only have to proof that 
\begin{equation}
...
\end{equation}
	...
	In Section \ref{sec: proofs}, for completeness, we will simplify the argument made by \cite{geiger1999elementary} and give a proof of \eqref{eq: convergence of conditional expectation}.
%end new
...
\begin{proof}[Proof of Theorem \ref{thm: Kolmogrov and Yaglom theorem}(\ref{thm:kolmogorov})]
	Denote by $B_n^j$ the event that the Galton-Watson process $(Z_n)$ survived up to generation $n$, and the left-most particle in the $n$-th generation is a descendant of $j$th particle of the first generation. 
	Then
\[\begin{split}
	&E[Z_n| Z_n>0]
...
\begin{proof}[Proof of Theorem \ref{thm: Kolmogrov and Yaglom theorem}\eqref{thm:yaglom}.]
	...
	%new added
	As have been discussed in Section \ref{sec: Methods}, we only have to proof that $\dot Z_n/n$ converge weakly to $\dot Y$.
	%end new added
