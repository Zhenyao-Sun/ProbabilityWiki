...
\end{equation}
	...
	In Section \ref{sec: proofs}, for completeness, we will simplify the argument made by \cite{geiger1999elementary} and give a proof of 
%YX \eqref{eq: convergence of conditional expectation}.
...
\end{equation}
	...
	%Therefore, if $Y$ is the weak limit of $Z_n/n$ conditioned on $\{Z_n > 0\}$, then $Y$ should satisfy \eqref{eq: x2 type size-biased equation for exponential distribution}, which suggests that $Y$ is exponentially distributed.
	Therefore, if the weak limit of $\dot Z_n/n$ exists, say $\dot Y$, then $\dot Y$ should satisfy \eqref{eq: x2 type size-biased equation for exponential distribution}, which, by Lemma \ref{lem: our equation}, suggests that \eqref{eq: convergence after size-biased} is true.
...
\end{equation}
	...
	In fact,
%deleted	
	%see \cite[Theorem 10.1]{ArratiaGoldsteinKochman2013},
%end deleted	
...
\end{equation*}
	...
	%$C_u$-th child of $u$, i.e. particle $uC_u$,
...
\end{itemize}
	...
	Using this analysis, we verify that
\begin{align*}
...
\begin{proof}[Proof of Theorem \ref{thm: change of measure}]
    ...
    Note that $\{(X_m( \mathbf t ))_{0\le m\le n}; {\mathbf G}_n\}  \overset{d}{=} (Z_m)_{0\le m\le n}$ and $\{(X_m( \mathbf t ))_{0\le m\le n};\ddot{\mathbf G}_n\}  \overset{d}{=} (\ddot Z_m)_{0\le m\le n}$.
...
\label{sec:spinesdecomposition}
	...
	%subsection
	section, we are now ready to
	%give the precise meaning that \eqref{eq: Our insight} admits a size-biased add-on structure for the size-biased $\mu$-Galton-Watson tree:
...
\end{equation}
	...
	%Here,  $(Z^{(\dot L - 1)}_m)_{m\in \mathbb N_0}$ denotes a $\mu$-Galton-Watson process with initial population distributed according to $\dot L - 1$.
...
%YX $(Z_n)$ 
    $(Z_n)_{n\geq 0}$ 
survived up to generation $n$, and the left-most particle in the $n$-th generation is a descendant of $j$th particle of the first generation.
	Then
\[\begin{split}
	&E[Z_n| Z_n>0]
...
\end{split}\]
	Here, $q_n = P[Z_n = 0] = f^{(n)}(0)$, $p_n = 1- q_n$ and $f$ is the probability generating function of offspring distribution $\mu$.
...
%YX	We proof this lemma by a contradiction.
    We prove this lemma by a contradiction.
	Assume that
\begin{equation}
...
\]
	...
	This, however, contradicts to \eqref{eq: contradiction}.
\end{proof}
...
%YX	Therefore, we can compare the distribution of $(\dot Z_n - 1)/n$
%YX	with that of $\dot Y$ using Lemma \ref{lem: compare}, which gives that
