...
\label{sec:model}
	...
	%$\{(Z_n)_{n\ge 0}; \mathbf P \}$ with $Z_0 = 1$ 
	$\{(Z_n)_{n\ge 0}; P \}$ with $Z_0 = 1$ 
%ZS: I replaced all the \mathbf P with P and all \mathbf E with E without marking them.
...
\cite{geiger1999elementary}, \cite{geiger2000new} and \cite{lyons1995conceptual}.
	...
	$E[g(X)]\in (0,\infty)$.
	%We say a random element $\{\dot X;\dot P\}$ is a $g(X)$-size-biased transform (or simply $g(X)$-transform) of $X$ if
...
\begin{equation*}
		%P[ g ( \ddot Z_1^{(n)}, \dots, \ddot Z_n^{(n)})]
		E[ g ( \ddot Z_1^{(n)}, \dots, \ddot Z_n^{(n)})]
	=
		%\frac{ P[ Z_n(Z_n-1) g( Z_1, \dots, Z_n)]} {P [ Z_n ( Z_n - 1)]}.
