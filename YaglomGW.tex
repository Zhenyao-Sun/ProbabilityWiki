...
\thanks{The research of the first author is supported in part by NSFC (Grant Nos. )}
%--------------------
\author{Renming Song}
\address{
	Renming Song\\
...
\item
	...
	%other children as unmarked particles.
...
\par	
	%The main reason for proposing such a model is that, if one writes $\ddot Z_m^{(n)}$ for the population of the $m$th ($1\le m\le n$) generation in the $k(k-1)$-type size-biased tree with height $n$, The process $(\ddot Z_m^{(n)})_{m=0,\cdots,n}$ can be obtained from the process $(Z_m)_{m=0,\dots,n}$ by a change of measure via the random variable $Z_n(Z_n-1)$. 
...
\par
	...
	%The present work is inspired by \cite{harris2015many}, the main purpose of our two-spine model is to characterize the $k(k-1)$-type size-biased branching process.
...
\par	
	...
	%some nonnegative random variable $X$. 
...
\label{eq:uniformly}
	...
	\to
	    1,
	\quad 
		%\uniformly s\in [0,\infty)\mbox{ as } n\to\infty.
...
%moved below
	%For any particle $u:=u_1\cdots u_{n-1}u_n$, we define its \defn{generation} as $\abs{u}:=n$ and its \defn{parent particle} as $\parent{u}:=u_1\cdots u_{n-1}$.
%end moved below
	For two particles 
	%$u=u_1\cdots u_n, v=v_1\cdots v_m\in\cU$,
	$u=u_1\dots u_n, v=v_1\dots v_m\in\cU$,
	$uv$ denotes the concatenated particle 
	%$uv:=u_1\cdots u_nv_1\cdot v_m$.
...
%end moved here
	...
	%\defn{number of offspring of $u$ in $\mathbf a$}
...
\end{equation*}
	...
	such that $v^{(0)}=\emptyset$ and $\parent{v^{(k)}}=v^{(k-1)}$ for any %$k=1,\cdots, |\tree|$. 
	$k=1,\dots, |\tree|$. 
	In the case that $|\tree|=\infty$, 
	%we simply write $k\in\bbN$ for $k=1,\cdots, |\tree|$.
...
\begin{equation*}
		\dot T
	:=
		%\set{u_0\cdots u_m\in\cU: m=0,\cdots,n,u_j\leq \tilde L_{u_0\cdots u_{j-1}},\forall j=1,\cdots,m},
...
\end{equation}
\par	
	The \defn{size-biased Galton-Watson measure $\dot \bG_n$} on %space
...
\item
    %$(\dot L_{v_0\cdots v_m},\dot C_{v_0\cdots v_m})=(l_{v_0\cdots v_m}(\tree),v_{m+1})$ for each $v_0\cdots v_mv_{m+1}\in\spine\cup\spine'$ with $k_n\neq m<n$ and
...
\end{multline*}
\par	
	The \defn{$k(k-1)$-type size-biased Galton-Watson measure $\ddot \bG_n$} on %space 
...
\begin{proof}[Proof of Theorem \ref{thm:changeofmeasure}]
    ...
    %$((X_m(\tree))_{m=0,\cdots,n};\ddot \bG_n) \eqlaw (\ddot Z_m)_{m=0,\cdots,n}.$
...
\end{equation*}
	...
	Since $|\dot V|=n$, we must have, for any particle $u\in\cU$, %$|\ancestor{u}\cap\dot V|\in\set{0,1,\cdots,n}$. 
...
\begin{equation*}
	...
	:=
		%\bigcup_{k=0}^n\set{u\in\ddot T: |\ancestor{u}\cap\expr{\ddot V\cup\ddot V'}|=k}.
...
\begin{equation*}
	...
	=
		%c^{-1}\int_0^1du\cdot\int_{kc}^{kc+\lambda} F(us)ds\leq\frac{\lambda}{c}, \quad \lambda\geq 0.
