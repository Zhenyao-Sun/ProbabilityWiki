...
\thanks{Supported in part by NSFC (Grant Nos. )}
%--------------------
\author{Renming Song}
\address{
	Renming Song\\
...
\thanks{Research supported in part by the Simons Foundation (\#429343, Renming Song)}
%--------------------
\author{Zhenyao Sun}
\address{
	Zhenyao Sun\\
...
\thanks{Supported by the China Scholarship Council.}
%----------------------------
%\keywords{[keywords]}
...
%with offspring distribution $\mu$. 
...
%		\sum_{k=0}^\infty k \mu\set{k}
	...
	= 1.
\end{equation}
	Suppose $L$ has %a non-degenerate 
	finite variance
\begin{equation}
\label{eq:variance}
		0	
...
%	\eqref{thm:kolmogorov} and \eqref{thm:yaglom} of Theorem \ref{thm:kesten}
...
%end new
%For probabilistic proofs using different methods, we refer readers to 
For probabilistic proofs of the above results, we refer our readers to 
%\cite{lyons1995conceptual} and \cite{geiger1999elementary}, \cite{geiger2000new}.
\cite{lyons1995conceptual}, \cite{geiger1999elementary} and \cite{geiger2000new}.
\par
%	\cite{lyons1995conceptual}
...
%\defn{size-biased distribution} of $L$, 
...
%new marked particle while leaving other children as unmarked particles.
...
\item
%	The system goes on and on. 
The evolution goes on.
\end{itemize}
\par
%	Notice all the marked particles form a descending family line 
Notice the marked particles form a descending family line
which will be referred as the \defn{spine}. 
Define $\dot Z_n$ as the population of the $n$th generation in the size-biased tree. %%\cite{lyons1995conceptual} proved that process 
It is proved in \cite{lyons1995conceptual} that the process
$(\dot Z_n)_{n\ge 0}$ is 
%the Doob's $h$-transformation of process $(Z_n)_{n=0..}$ with respect to the martingale $(Z_n)_{n\ge 0}.$ 
...
\end{equation}
\par
%	In this article, we present a $k(k-1)$-type size-biased Galton-Watson tree 
In this note, we present a $k(k-1)$-type size-biased Galton-Watson tree 
%	whose structure relies on a two-spines skeleton. 
...
\item
%	Any marked particle at generation $K_n$, however, gives an independent birth 
...
%as new marked particles while leaving other children as unmarked particles.
...
%	\defn{two-spines skeleton} with $K_n$ as the last generation at which those two spines {\bf coincide ?} with each other.
\defn{two-spine skeleton} with $K_n$ as the last generation where the two spines are
together.
\par	
%	The main reason of promoting such a model is that, 
The main reason for proposing such a model is that,
%	if one write $\ddot Z_m^{(n)}$ for the population of the
if one writes $\ddot Z_m^{(n)}$ for the population of the
$m$th ($1\le m\le n$)
generation in the $k(k-1)$-type size-biased tree with height $n$, 
%there is a change of measure relationship between process
...
%	Let $(Z_m)_{m\ge 0}$ be the $\mu$-Galton-Watson branching process 
...
\end{thm}
\par
%The idea of considering branching particle system 
The idea of considering a branching particle system 
with more than one spine is not new.
%A related construction 
%in  other situation occurs in \cite{harris2015many}. They developed a branching particle system with $k$-spines as the key element to state a $k$th moment formula for branching Markov processes and branching random walks known as the many-to-few formula. 
...
%Though highly inspired by their work, we employ a different two-spines model for the purpose to characterize the $k(k-1)$-type size-biased branching process.
...
%	\cite{lyons1995conceptual} argued that, 
It was shown in \cite{lyons1995conceptual} that,
%	in the size-biased Galton-Watson tree, 
...
%$k=1..n$ where $(Z'_n)_{n=0..}$ represent
$k=1, \dots, n$, where $(Z'_n)_{n\ge0}$ is
%a $\mu$-Galton-Watson branching process with initial population 
...
%We prefer to rewrite equation \eqref{eq:rawdecomposition} into
...
%	Let $(\dot Z_n)_{n\ge 0}$ be the size-biased $\mu$-Galton-Watson branching process. 
...
\end{prop}
\par
%	With this proposition, we then discuss the asymptotic behaviour of $\dot Z_n$'s distribution. 
...
%passing $n\to\infty$ in equation \eqref{eq:twospinedecomposition}, 
...
\end{equation}
%Solving equation \eqref{equation:exponential}, 
...
%Yaglom's law using Proposition \ref{lem:twospinedecomposition}.
...
\end{equation*}
%where we take $\bbN=\{1,2,\cdots\}$. 
where $\bbN:=\{1,2,\cdots\}$. 
%For two particles $u, v\in\cU$, $uv$ denotes the concatenated particle $(u\emptyset = \emptyset u = u)$, and therefore
...
%A \defn{planer tree} $\tree$ is defined 
...
%          l_{(u_0u_1\cdots u_{j-1})}, \forall j=1,\cdots, m}.
          l_{u_0u_1\cdots u_{j-1}}, \forall j=1,\cdots, m}.
\end{equation*}
%A \defn{spine} $\spine$ on a planer tree $\tree$ 
...
for any $k=1,\cdots, |\tree|$. 
%In the case that $\|\tree\|=\infty$, we simply write $k=1,\cdots,\|\tree\|$ for $k\in\bbN$.
...
%while conditioning on $\sigma(\dot L)$, 
conditioned on $\dot L$,
is uniformly distributed on $\{1,\cdots,\dot L\}$. 
%Independent of $(\dot L_u)_{u\in\cU}$, 
...
%as the building elements to construct the size-biased Galton-Watson tree 
...
%the $C_u$th children of $u$, i.e. particle $(u,C_u)$, 
...
\par
%	We then consider the distribution of the $\dot\bbT_n$-valued 
...
\end{equation}
\par	
%	Equation \eqref{eq:treespinemeasure}, \eqref{eq:sizebiasedGWmeasure} and their natural corollary \eqref{eq:htransformation} 
...
%while conditioning on $\sigma(\ddot L)$, is uniformly distributed on 
conditioned on $\ddot L$, is uniformly distributed on 
$\{(i,j)\in\bbN^2:1\leq i\neq j\leq \ddot L\}$. 
%Suppose that $(\dot L_u)_{u\in\cU}, (\dot L_u,\dot C_u)_{u\in \cU}$, 
Suppose that $(L_u)_{u\in\cU}, (\dot L_u,\dot C_u)_{u\in \cU}$, 
$(\ddot L,\ddot C,\ddot C')$ and $K_n$ are independent of each other. 
%We then use these elements to build the $k(k-1)$-type size-biased 
...
\par
%We then consider the distribution of the random element 
...
%		\bP\set{(\ddot T,\ddot V,\ddot V')=(\tree,\spine,\spine')}
...
%under $\bG_n$ is exactly a Galton-Watson branching process 
...
%Rewrite equation \eqref{eq:k(k-1)typesizebiasedGWmeasure} into its Radon-Nikodym derivative form,
...
Since $|\dot V|=n$, we must have, for any particle $u\in\cU$, 
%$\|\ancestor{u}\cap\dot V\|\in\set{0,1,..,n}$. 
...
%$\|\ancestor{u}\cap(\ddot V\cup \ddot V')\|\in\{0,1,..,n\}$. 
...
\end{itemize}
%Here, $(Z_k')_{k\ge 0}$ and $(Z_k'')_{k\ge 0}$ are $\mu$-Galton-Watson branching 
...
%$\mu$-Galton-Watson branching process $(\dot Z_k)_{k\ge 0}$. 
$\mu$-Galton-Watson process $(\dot Z_k)_{k\ge 0}$. 
Following those arguments, taking Laplace transform, 
%we may rewrite \eqref{eq:rawtwospinedecomposition} into,
...
\end{multline}
%Taking \eqref{eq:keytakingback} back into 
...
%    and function $F:[0,\infty)\to [0,1]$ satisfies that, 
...
,\quad \lambda\geq 0.
\end{equation*}
%Taking this new upper bound back into the right hand 
...
%Actually, while $F=0$ on $[0,kc)$, we have
...
%	For the Galton-Watson branching process $((Z_n)_{n\ge 0};\prob)$, 
For the Galton-Watson process $((Z_n)_{n\ge 0};\prob)$, 
	write $\prob_n^*[\cdot]:=\prob[\cdot|Z_n>0]$. 
%	By Kolmogorov's estimate, \eqref{thm:kolmogorov} of Theorem \ref{thm:kesten}, 
...
\end{equation*}
	As a consequence, 
%	laws of $(\frac{Z_n}{n};\prob_n^*)_{n\ge 1}$ are tight and 
$(\frac{Z_n}{n};\prob_n^*)_{n\ge 1}$ is tight and 
%there will be some sub-sequence, say 
...
\end{equation}
	Taking $n=n_k$ and
letting $k\to\infty$,
%it follows from Kolmogorov's estimate, 
...
%	In particular, $Y$'s distribution is independent of the choice of $n_k$, 
