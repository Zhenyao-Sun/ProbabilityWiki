...
%ZY% \SHORTTITLE{Introduction to the \emph{ejpecp} Class} 
	\SHORTTITLE{A 2-spine decomposition and Yaglom's theorem}

%ZY% \TITLE{Introduction to the \emph{ejpecp} Class Version 1.5\thanks{Supported by the Institute of Mathematical Statistics (IMS) and the Bernoulli Society.}\
%ZY% \thanks{Current maintainer of class file is
%ZY% \href{http://www.vtex.lt}{VTeX, Lithuania}. Please send all queries to
%ZY% \href{mailto:latex-support@vtex.lt}{\texttt{latex-support@vtex.lt}}.}} % \thanks is optional. Insert line breaks with \\
\TITLE{
...
%ZY% \AUTHORS{%
%ZY% Krzysztof~Burdzy\footnote{University of Washington, United States of America.
%ZY% \EMAIL{burdzy@math.washington.edu}}
%ZY% \and %% remove this line and below if single author
%ZY% Djalil~Chafa\"{\i}\footnote{Universit\'e Paris-Dauphine,
%ZY% France. \BEMAIL{djalil@chafai.net} \url{http://djalil.chafai.net}}}%AUTHORS
\AUTHORS{
	Yan-Xia~Ren
\footnote{
...
%ZY% \ABSTRACT{The $\LaTeXe$ class \emph{ejpecp} is designed for typesetting of articles to be published in the research periodicals \emph{Electronic Journal of Probability} (EJP) and \emph{Electronic Communications in Probability} (ECP).}
...
%ZY: The body of the original sample.tex is deleted without  any mark.
...
\end{lemma}	
	...
	%ZY% is distributed approximately like $\dot Z_{ \floor{U\cdot n}}$.
...
\[\label{eq: Our insight}
	\frac{\ddot Z_n^{(n)}}{n}
	%ZY% \overset{d} \approx \frac{\dot Z_n}{n} + U \cdot \frac{   \dot Z'_{  \floor{ U n }  }   }    {   Un   },
...
\end{equation}
	...
	%ZY% they then show that $\ceil{U \cdot \dot Z_n}$ is distributed approximately like $Z_n$ conditioned on $\{Z_n > 0\}$.
...
Therefore, if $\{Z_n/n; P(\cdot| Z_n > 0)\}$ converges weakly to a random variable $Y$, then $Y$ should satisfy \eqref{eq: Geiger's equation}, which suggests that $Y$ is exponentially distributed.
	...
	%ZY% but does not imply the convergence of $Z^{(i)}_{ \floor{Un_k}}/Un_k, i=1, 2$.
...
\begin{proposition}\label{prop: size-biased add-on of size-biased tree }
	...
	%ZY% = E [e^{-\lambda \dot Z_n}] E[g(\lambda, \floor{Un})e^{-\lambda \dot Z_{\floor{Un}}}],
...
\begin{proof}[Proof of Theorem \ref{thm: Kolmogrov and Yaglom theorem}\eqref{thm:yaglom}.]
	...
	%ZY% = E [e^{-\lambda \frac{\dot Z_n - 1}{n}}] E\big[g\big(\frac{\lambda}{n}, \floor{Un} \big)e^{-\lambda U \frac{\dot Z_{\floor{Un}}}{Un} }\big],
...
%ZY% Moved to the end
%ZY% \noindent
%ZY% {\bf Acknowledgements:}  We thank the referee for helpful comments on the first version of this paper.
%ZY% \vspace{.1in}
%ZY% END Moved
...
%ZY: The original bibitems were deleted and replaced by our bibitems without any marks
%ZY: As recommend I changed \emph{} to {\it }; and \textbf{} to {\bf }. Also as recommend, I added the MR number and ARXIV number, without furthwr marks

%ZY%: The following are two example fram sample.tex:

%ZY% \bibitem{doob} Doob, J. L.: Heuristic approach to the Kolmogorov-Smirnov theorems. \emph{Ann. Math. Statistics} \textbf{20}, (1949), 393--403. \MR{0030732}

%ZY% \bibitem{grisha} Perelman, G.: The entropy formula for the Ricci flow and its geometric applications, \ARXIV{math.DG/0211159}
...
%ZY: I am not sure the following entry is correct.
\bibitem{geiger2000new}
  Geiger, J.:
  A new proof of Yaglom's exponential limit law.
  %ZY% In \emph{Mathematics and Computer Science},
...
\bibitem{kesten1966galton}
  ...
  %\emph{Theory Probab. Appl.} %ZY: The information on google and mathscinet is not the same. I used the information on mathscinet.
...
\bibitem{RenSongSun2017Spine}
  ...
  %\emph{arXiv preprint arXiv:1711.09188}
  %(2017).
 \ARXIV{1711.09188}
...
\bibitem{yaglom1947certain}
  ...
  %\emph{Doklady Akad. Nauk SSSR (NS)}
  \emph{Doklady Akad. Nauk SSSR (N.S.)}
  \textbf{56} (1947), 795--798.
  \MR{0022045}

%ZY: The following entry also has different information from google and mathscinet. I used mathscinet.
\bibitem{Zubkov1975}
  Zubkov, A. M.:
...
%ZY% Deleted
%ZY% \ACKNO{We are grateful to Martin Hairer who provided a nice \texttt{MR} macro and to S\'ebastien Gou\"ezel for his useful comments on the internals of the class file.}
%ZY% End Deleted

%ZY% moved and changed below
\ACKNO{
